...
export async function getProducts(req, res) {
    try {
        ...
        response.prevLink = response.hasPrevPage ? (urlTest + `/products?page=${response.prevPage}&limit=${response.limit}`) : null
        response.nextLink = response.hasNextPage ? (urlTest + `/products?page=${response.nextPage}&limit=${response.limit}`) : null
...
export async function getProductsRealTime(req, res) {
    try {
        ...
        response.prevLink = response.hasPrevPage ? (urlTest + `/products/realTimeProducts?page=${response.prevPage}`) : null
        response.nextLink = response.hasNextPage ? (urlTest + `/products/realTimeProducts?page=${response.nextPage}`) : null
        response.isValid = !(xpage <= 0 || xpage > response.totalPages)
